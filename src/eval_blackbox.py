...
precoder = sys.argv[1]
model_to_num = sys.argv[3]
if model_to_num == '1':
    model_from_num = '2'
elif model_to_num == '2':
    model_from_num = '1'
print(f"Apply attack generated from model {model_from_num} onto model {model_to_num}\nprecoder: {precoder}\nattack type: {attack_type}")
...
for epsilon in epsilons:
    ...
    for cell_index in cells:
        model_from = load_model(
            f'../saved_nn_models/model_{model_from_num}/ORIG_NN_{precoder}_maxprod_cell_{cell_index}.h5', 
            custom_objects={'rel_mse':rel_mse, 'rmse':rmse})
        model_to = load_model(
            f'../saved_nn_models/model_{model_to_num}/ORIG_NN_{precoder}_maxprod_cell_{cell_index}.h5', 
            custom_objects={'rel_mse':rel_mse, 'rmse':rmse})
        # regardless of what the attack is, always use the DAE model trained with PGD attck 
        model_dae = load_model(
            f'../saved_nn_models/model_{model_to_num}/DAE_{precoder}_PGD_maxprod_cell_{cell_index}.h5')
        model_adv_regressor = load_model(
            f'../saved_nn_models/model_{model_to_num}/ADV_REGRESSOR_0025_{precoder}_{attack_type}_maxprod_cell_{cell_index}.h5',
            custom_objects={'rel_mse':rel_mse, 'rmse':rmse})
        model_adv_regressor_from = load_model(
            f'../saved_nn_models/model_{model_from_num}/ADV_REGRESSOR_0025_{precoder}_{attack_type}_maxprod_cell_{cell_index}.h5',
            custom_objects={'rel_mse':rel_mse, 'rmse':rmse})

        ### evaluate case 1: original model + original input 
        if (epsilon == epsilons[0]):
            output_NN = model_to.predict(x_test)
        ...
        if attack_type == "PGD":
            ...
            x_adv = pgd(model=model_from, data=x_test, alpha=epsilon/nb_iterations, nb_iterations=nb_iterations)
            # attack for the adversarially trained model
            x_adv_adv_regres = pgd(model=model_adv_regressor_from, data=x_test, alpha=epsilon/nb_iterations, nb_iterations=nb_iterations)
        elif attack_type == "FGSM":
            # attack for original DL model and DAE defense
            x_adv = fgsm_loss(model=model_from, data=x_test, epsilon=epsilon)
            # attack for the adversarially trained model
            x_adv_adv_regres = fgsm_loss(model=model_adv_regressor_from, data=x_test, epsilon=epsilon)

        ### evaluate case 2: original model + adversarial input
        output_NN_adv = model_to.predict(x_adv)
        ...
        output_NN_adv_dae = model_to.predict(input_denoised)   # (5000, 6)
...
if not os.path.isdir(f'../eval_output/black_box_from_{model_from_num}_to_{model_to_num}'):
    os.makedirs(f'../eval_output/black_box_from_{model_from_num}_to_{model_to_num}')
if not os.path.isdir(f'../eval_output/black_box_from_{model_from_num}_to_{model_to_num}/{precoder}_{attack_type}'):
    os.makedirs(f'../eval_output/black_box_from_{model_from_num}_to_{model_to_num}/{precoder}_{attack_type}')
np.savetxt(f"../eval_output/black_box_from_{model_from_num}_to_{model_to_num}/{precoder}_{attack_type}/epsilons.txt", epsilons)
np.savetxt(f"../eval_output/black_box_from_{model_from_num}_to_{model_to_num}/{precoder}_{attack_type}/base.txt", base)
np.savetxt(f"../eval_output/black_box_from_{model_from_num}_to_{model_to_num}/{precoder}_{attack_type}/adv.txt", adv)
np.savetxt(f"../eval_output/black_box_from_{model_from_num}_to_{model_to_num}/{precoder}_{attack_type}/adv_dae.txt", adv_dae)
np.savetxt(f"../eval_output/black_box_from_{model_from_num}_to_{model_to_num}/{precoder}_{attack_type}/adv_regressor.txt", adv_regress)
